...
링크를 누르면 **닫힌 카드가 3D로 열리는** 모바일 초대장입니다. 앞장을 누르면 그 장이
왼쪽으로 넘어가며 열리고, 그 아래에서 초대장이 드러납니다 — 열기 전에는 스크롤이 잠깁니다.
...
components/invite/
  ...
  sections.tsx    초대글·일시·달력·갤러리·오시는길·연락처·회신·계좌·안내
  reveal.tsx      스크롤 등장 (IntersectionObserver, 외부 라이브러리 없음)
  stage.tsx       카드를 놓는 자리와 돌아가는 링크
...
- **여는 동작이 먼저** — 카드가 열리기 전에는 스크롤이 잠깁니다. 초대장은 받아서
  여는 물건이라, 링크를 누르자마자 글이 좌르륵 나오면 카드가 아니라 웹페이지입니다.
- **덮개는 왼쪽으로** — 책과 같은 방향입니다. 반대로 만들면 오른쪽에서 왼쪽으로
  읽는 책이 됩니다.
- **과하지 않은 움직임** — 여는 연출과 스크롤 등장(투명도 + 20px 상승)뿐이고,
