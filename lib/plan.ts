...
/**
 * 청첩장 "발행"(내 청첩장 주소를 만들어 하객에게 보내기)이 열렸는지.
 *
 * 지금은 false 입니다. 정적 배포라 사용자가 만든 청첩장의 HTML 을 새로
 * 찍을 수 없고, 카카오톡 링크 미리보기는 크롤러가 자바스크립트를 돌리지
 * 않아 청첩장별 HTML 이 미리 있어야만 뜹니다. 그래서 지금 할 수 있는 것은
 * "만들고 미리 보는 것"까지입니다.
 *
 * 화면 문구가 이 값을 보고 갈리게 해 두었습니다. 발행·결제가 실제로
 * 붙는 날 이 값만 true 로 바꾸면 안내가 한 번에 따라옵니다.
 * 켜기 전에 필요한 것: 요청 시 렌더가 되는 호스팅, 청첩장 저장소,
 * 수정용 링크(편집 토큰), 결제 연동, 그리고 PREMIUM_SECTIONS 잠금 적용.
 */
export const PUBLISHING_ENABLED = false;

/**
 * 원화. 한 청첩장에 한 번 결제하면 예식이 끝날 때까지 유지됩니다.
 * 발행이 열리기 전까지는 "예정 가격"이며 결제를 받지 않습니다.
 */
