...
/* ════════════════════════════════════════════════════════════
   ...
   화면 순서는 휴대폰을 기준으로 짰습니다.
     ① 카드가 먼저 보이고 (받는 사람이 볼 그 카드 그대로입니다)
     ② 아래에서 디자인을 고르고 내용을 채우고
     ③ 맨 아래 고정된 단추로 보냅니다.
   ════════════════════════════════════════════════════════════ */
...
export function Maker({ initial }: { initial: InviteData }) {
  ...
  /* 적다 만 초안을 되살립니다. 효과(effect)가 아니라 ref 로 하는 것은
     첫 그림과 브라우저 그림이 어긋나지 않게 하려는 것입니다. 같은
     디자인의 초안일 때만 되살립니다 — 다른 카드를 고른 사람에게
  ...
  return (
    <div ref={restore}>
      {/* ── ① 카드 ── */}
      <section className="bg-cream/60 pt-8 pb-12">
        <div className="shell">
          ...
          <div className="mt-6">
            {/* key 를 걸어 디자인을 바꿀 때마다 닫힌 상태에서 다시 시작합니다.
                화살표 키는 끕니다 — 아래 입력칸을 오가는 키와 부딪칩니다. */}
            <FoldedCard key={data.d} data={data} keyboard={false} />
          </div>
        </div>
      </section>
                ...
                return (
                  <li key={d.id} className="shrink-0 snap-start">
                    <button
                      ...
                      onClick={() => set("d", d.id)}
                      aria-pressed={on}
                      className={`block w-[7rem] rounded-md p-1.5 transition-colors ${
                        on ? "bg-rose-mist" : "hover:bg-sand"
                      }`}
                    >
                      {/* 고르는 칸에도 실제 표지가 그대로 들어갑니다 */}
                      <ClosedCard design={d} data={data} />
                      <span
                        className={`mt-1 block text-center text-[0.6875rem] ${
      ...
      <section className="mt-section">
        <div className="shell max-w-narrow">
          ...
          <p className="mt-2 text-caption text-ink-soft">
            고친 자리는 바로 위 카드에 그대로 반영됩니다. 표지 · 인사말 ·
            일정이 각각 카드의 한 면입니다.
          </p>
          ...
          <div className="mt-block grid gap-5">
            ...
            <Field label="초대 글" hint="카드를 열면 왼쪽 면에 실립니다.">
              <textarea
            ...
            <Field label="연락처" hint="비우면 «전화» 단추가 사라집니다.">
              <input
                type="tel"
                inputMode="tel"
                className={INPUT}
